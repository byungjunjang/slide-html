...
테마 가이드 마크다운(필수)을 사용자가 제공하면, 토큰을 추출 → `theme.json`(v1 컨트랙트) → `colors_and_type.css` + `_pptx-slide.css` + 보일러플레이트 37장까지 일괄 렌더링합니다. 새 폴더로 추가될 뿐 **기존 프리셋은 손대지 않습니다** (멀티 프리셋 모델).
