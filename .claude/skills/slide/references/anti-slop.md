...
3. **§13 비주얼=근거 규율 self-check** — card-row 반사 / step-flow 장식 / 장식 비주얼 + 측정 쿼터(1메시지 · ≤3불릿 · 여백≥30% · 우상단 공백 · 사진 단독)
4. **§11 활성 프리셋 시그니처 self-check** — preset 정체성 보존 확인
5. **§12 미세 서식 self-check** — `references/text-formatting-rules.md` 통과
6. 위반 발견 시 슬라이드 재할당 또는 어휘 교체
7. 재빌드 후 PPTX 검증
...
## 13. 비주얼=근거 규율 (bytonylee 벤치마크 · warn)

> **목표**: ① 카드 반복 탈피 ② 비주얼을 "장식"이 아니라 **핵심 메시지의 근거/설명**으로 (P1). 본 섹션은 `PIPELINE_UPDATE_PLAN.md` 항목 G의 **warn-level 자기 점검** — 위반이 (아직) 빌드를 막지는 않지만, 데크가 "매번 비슷한 카드 도배"로 회귀하는 가장 흔한 경로다. Phase 4에서 일부 항목이 빌드 게이트(B)로 승격된다. **이 섹션이 bytonylee 취향 규칙의 SSOT** — DESIGN.md §10은 여기를 가리킨다.

### 13.1 명명된 안티패턴 (reflexive layout 금지)

- ❌ **card-row 반사** — 새 슬라이드의 기본 레이아웃으로 "동일 카드 N개 가로 나열"을 먼저 떠올리는 것. 카드 그리드는 **진짜 병렬 항목**(서로 대등한 N개)일 때만 정당하다. 그 외에는 Visual-Primary / Editorial / Hero 어휘를 먼저 고려. (cf. §1 Density 50% cap, DESIGN.md §5 카테고리 회전)
  - 픽스: "이 N개가 정말 대등한 병렬인가?" 아니면 → `numbered-progression`(순서) / `paired-concept`(2축) / `annotated-screenshot`(증거)로 재작곡.
- ❌ **step-flow 장식** — 실제 시간/인과 순서가 없는데 `1 → 2 → 3` 화살표 플로우로 그리는 것. `process-arrow`는 **진짜 단계적 흐름**일 때만. (cf. DESIGN.md §5 Category E)
  - 픽스: 순서가 없으면 `overview-cards` 또는 `paired-concept`. 순서가 있으면 화살표가 의미를 가진다.
- ❌ **장식 비주얼 (decorative-visual)** — 메시지를 설명·증명하지 않는 시각 요소: 배경 도형 / halo / dot / 추상 그래픽, "빈 칸 채우기용" 일러스트, 의미 없는 아이콘 나열. **모든 비주얼은 근거여야 한다** (P1). (cf. §8 캐릭터/데코, DESIGN.md §9)
  - 픽스: 비주얼을 지우거나, 메시지를 **증명하는** 비주얼(차트 · 다이어그램 · UI 캡쳐 · 관계도)로 교체.

### 13.2 측정 쿼터 (warn)

- ❌ **한 슬라이드 2개 이상 핵심 메시지** — 1 슬라이드 = 1 메시지. (cf. §7)
- ❌ **본문 불릿 4개 이상** — 줄글 불릿 리스트는 ≤3. 4개 이상이면 카테고리화 후 분할하거나 카드/표로 재구조화. (카드 그리드 항목 cap은 §7의 6개와 별개 — 여기선 "줄글 불릿 리스트" 한정)
- ❌ **여백 < 30%** — 슬라이드 면적의 ≥30%는 의도적 빈 공간(breathing room). 콘텐츠가 캔버스를 꽉 채우면 "고밀도"가 아니라 "답답함".
- ❌ **우상단 코너가 꽉 참** — page counter 외에 우상단 영역은 비워 시선 진입로를 확보. (chrome의 page counter는 예외)
- ❌ **사진을 썸네일로 곁들임** — 사진을 쓸 거면 **지배 요소로** (full-bleed 또는 ≥~55% 점유, cf. P1 · evidence-slots). 텍스트 옆 작은 장식 사진은 13.1의 장식 비주얼이다.

### 13.3 self-check
데크 작성 마무리 직전, 위 항목을 **데크 전체**에 적용. 위반 슬라이드는 §10 흐름대로 재작곡한다. 빌드 게이트(B, `build.mjs`)가 정착하면 13.1·13.2 일부가 WARN으로 자동 출력된다.
