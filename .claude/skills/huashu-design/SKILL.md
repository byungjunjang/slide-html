...
> 🛑 **슬라이드/PPT/프레젠테이션/.pptx 작업은 즉시 `/slide` 스킬로 위임할 것** — 사용자가 슬라이드 만들어달라고 하면 (한국어 "슬라이드", 영어 "slide / presentation / deck", 中文 "幻灯片" 모두 포함) huashu-design 의 Reveal/PDF/디자인 변체 등 어떤 슬라이드 경로도 사용하지 말고 `/slide` 가 처리한다. huashu-design 의 슬라이드 관련 가이드 (`references/slide-decks.md`, `references/editable-pptx.md`, `scripts/export_deck_pptx.mjs`, `scripts/export_deck_pdf.mjs`, `scripts/export_deck_stage_pdf.mjs`, `scripts/html2pptx.js`) 는 `/slide` 스킬의 내부 의존성으로만 호출되며, huashu-design 단독으로는 슬라이드 작업을 시작하지 않는다. 이 doctrine 은 사용자 명시 선호 (memory feedback "PPTX는 항상 editable-only 경로") 에 의해 강제됨.
>
> huashu-design 이 단독으로 처리하는 작업은: 인터랙티브 프로토타입, 디자인 변체 탐색, 정보 그래픽, 애니메이션 데모, MP4/GIF 비디오 export, App 프로토타입, 디자인 방향 컨설팅, 5차원 평론 등 — **슬라이드를 제외한** 시각 산출물.
...
1. **理解需求**：
   ...
   - 🛑 **幻灯片/PPT 任务（2026-04-24 强化）：editable PPTX 是默认交付形式**：
     - **源文件**：每页独立 HTML + `assets/deck_index.html` 聚合（重命名为 `index.html`，编辑 MANIFEST 列所有页），浏览器演示版是 source of truth
     - **默认输出 = editable PPTX**：HTML **从第一行就按 4 硬约束写**（见 `references/editable-pptx.md`），通过 `scripts/export_deck_pptx.mjs` + `html2pptx.js` 转换。PowerPoint/Keynote 里双击即可编辑文字——**这是基准交付**
     - **4 硬约束速查**：① body 1280×720px · 所有元素 `position: absolute`；② DIV 禁止直接含文字，必须用 `<p>` / `<h1>-<h6>` / `<ul>/<ol>` 包裹；③ 背景/边框/阴影只放在 DIV 上，不放在 p/h*/ul；④ 无 CSS gradient，无 `background-image`（用 `<img>` 标签替代）
     - **事后从视觉驱动 HTML 硬转** = pass 率 < 30%，不要做；要么开工就按 editable 写，要么走图片 PPTX fallback
     - **图片 PPTX 仅在 fallback 场景用**：(a) 页数 > 30 改写成本过高；(b) 视觉自由度是核心（动画 / web component / 复杂 SVG）；(c) 用户明确说不需要编辑。Fallback 流程见 `references/editable-pptx.md` 的「Fallback」节
     - **PDF**（`export_deck_pdf.mjs`）用于存档或纯演讲场景
     - **≥ 5 页 deck 必须先做 2 页 showcase 定 grammar 再批量推**
     - 详见 `references/slide-decks.md` + `references/editable-pptx.md`
...
### 프로젝트 폴더 격리 원칙 (2026-04-24 강화 · 위반 금지)

**모든 작업·산출물은 `<workspace-root>/projects/<project-name>/` 하위에 완결된 구조로 격리**. 루트/홈/Downloads/임의 위치에 파일 흩뿌리지 말 것.

#### 표준 프로젝트 폴더 구조

```
projects/<project-name>/
├── index.html                      # deck navigator (슬라이드 작업 시)
├── design-system/                  # jangpm 등 디자인 프리셋 심볼릭 링크
├── slides-editable/                # editable 4 조건 준수 HTML (슬라이드 기본)
│   ├── _base.css                   # 폰트 로드 + body 1280×720 공통 베이스
│   └── 01-*.html ... 09-*.html
├── slides/                         # (선택) 시각 자유도 소스 HTML, html 쇼케이스용
├── build-pptx-editable.cjs         # editable PPTX 빌드 스크립트 (기본)
├── html2pptx.js                    # skill scripts/ 에서 복사 (require resolve용)
├── package.json                    # 프로젝트 로컬 의존성 (playwright/pptxgenjs/sharp)
├── node_modules/                   # npm install
├── artifacts/                      # 중간 산출물 (비최종 · git ignore 대상)
│   ├── exports/                    # Playwright 스크린샷 원본
│   ├── playwright-mcp/             # MCP 내비 YAML / console 로그
│   └── *-test.pptx                 # 검증용 단일 슬라이드 테스트 빌드
├── <name>-editable.pptx            # ★ 최종 교부물: editable PPTX (기본)
├── <name>-editable.pdf             # 시각 검증용 PDF (LibreOffice 변환)
└── <name>.pptx                     # (선택) fallback 이미지 PPTX
```

#### Playwright MCP 스크린샷 저장 규칙 (필수)

`browser_take_screenshot` 의 `filename` 은 **반드시 `artifacts/` 상대 경로**로 지정:
- ❌ `filename: "cover.png"` → 작업 디렉토리 루트에 생성됨 → **금지**
- ✅ `filename: "projects/<project>/artifacts/screenshots/cover.png"`
- 작업 종료 시 자동 생성된 `.playwright-mcp/` 폴더를 `projects/<project>/artifacts/playwright-mcp/` 로 이동

#### HTTP 서버 실행 규칙

`python3 -m http.server` 는 반드시 `--directory <project-root>` 플래그로 프로젝트 폴더 고정:
```bash
python3 -m http.server 8765 --directory /path/to/projects/<project-name> &
```

### 기타 파일 규칙

- HTML 파일명 서술적: `Landing Page.html`, `iOS Onboarding v2.html`
- 대개편 시 구버전 보존: `My Design.html` → `My Design v2.html`
- > 1000 행 대형 파일은 여러 JSX 로 분할 import
- 고정 크기 콘텐츠(슬라이드/애니메이션)는 play position localStorage 저장 — 새로고침 유지
- 최종 산출물은 브라우저 열기 또는 Playwright 로 검증 (artifacts/ 에 스크린샷 보관)
