...
> **slide-html 안에서는 이 파일이 SSOT가 아니다.** `/slide` 파이프라인/데크에 다이어그램을 넣을 때 스킨 SSOT는 **활성 프리셋**(`../../slide/assets/design-systems/active.json` → `<preset>/colors_and_type.css`)이다. 다이어그램은 데크의 `design-system/colors_and_type.css`를 `<link>` 해 `var(--accent)`/`var(--text)` 등 **프리셋 CSS 변수**를 참조하고, 아래 의미역(`paper`/`ink`/`accent`…)은 그 변수로 매핑된다. 매핑표·계약: [`../../slide/references/diagram-slots.md`](../../slide/references/diagram-slots.md), SKILL §0.5. 아래 기본 토큰/온보딩은 **리포 밖 standalone 사용 시에만** 유효.
