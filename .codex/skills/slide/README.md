...
```bash
bash .codex/skills/slide/scripts/init-project.sh <project-name> [<preset>]
...
체계적 기획이 필요하면 별도 `slide-plan` 스킬(같은 리포의 `.codex/skills/slide-plan/`)이 먼저 `slide_plan.json`을 산출하고, 본 스킬이 그걸 입력으로 받아 R2(차트↔takeaway 일체화)/R5(evidence 매핑)을 빌드 시점에 검증합니다.
...
별도 `theme-init` 스킬(`.codex/skills/theme-init/`)이 새 프리셋을 만들면 결과를 이 번들의 `assets/design-systems/<new-preset>/`에 직접 떨구고, `assets/design-systems/README.md` 카탈로그를 자동으로 갱신합니다.
