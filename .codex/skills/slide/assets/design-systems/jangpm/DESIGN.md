...
### 비주얼=근거 (bytonylee 벤치마크 · warn)

> 상세·픽스는 `references/anti-slop.md` §13이 SSOT. 요지만:

- ❌ **card-row 반사** — "동일 카드 N개 가로 나열"을 기본 레이아웃으로 반사. 진짜 병렬 항목일 때만 카드 그리드.
- ❌ **step-flow 장식** — 실제 순서 없는 `1→2→3` 화살표 플로우.
- ❌ **장식 비주얼** — 메시지를 증명하지 않는 시각 요소(배경 도형 · 의미 없는 아이콘 · 빈 칸 채우기 일러스트). 모든 비주얼은 근거.
- ❌ **측정 쿼터 위반** — 한 슬라이드 2+ 메시지 / 본문 불릿 4+ / 여백 < 30% / 우상단 꽉 참 / 사진을 썸네일로 곁들임(사진은 ≥~55% 지배).
